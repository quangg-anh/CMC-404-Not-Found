...
|---|---|---|
| `VanBanPhapLuat` | `so_hieu` + `ngay_ban_hanh` | ten, loai_vb, co_quan, ngay_hieu_luc, trang_thai, `visibility` |
...
| `BaiTomTat` | uuid | tieu_de, noi_dung_binh_dan, media_type, `status: draft\|review\|published\|archived`, citations[] |
| `DeXuatDinhChinh` | uuid | draft_text, alert_ids[], khoan_ids[], `status`, created_by |
...
|---|---|---|
| `POST` | `/admin/ingest/legal` | đẩy VB vào pipeline |
| `GET` | `/admin/legal/van-ban/{id}` | cây Điều/Khoản + entities |
| `GET` | `/admin/legal/khoan/{id}` | chi tiết Khoản |
| `POST` | `/admin/legal/diff` | so sánh VB/Khoản |
| `POST` | `/admin/qa/ask` | QA nội bộ |
| `GET` | `/admin/jobs/{id}` | trạng thái job |
...
| `POST` | `/admin/link/preview` | dry-run link bài↔Khoản |
| `POST` | `/admin/briefs/generate` | sinh tin tóm tắt draft |
| `GET` | `/admin/briefs` | danh sách draft/review/published |
| `POST` | `/admin/briefs/{id}/publish` | PublishGate |
| `POST` | `/admin/suggestions/generate` | gợi ý đính chính từ alert |
| `PATCH` | `/admin/suggestions/{id}` | chỉnh draft |
...
| Method | Path | Mục đích |
|---|---|---|
| `GET` | `/citizen/news` | tin `published` (phân trang) |
| `GET` | `/citizen/news/{id}` | chi tiết + citations |
| `POST` | `/citizen/qa/ask` | chatbot có citation |
| `GET` | `/citizen/legal/van-ban` | VB công khai |
| `GET` | `/citizen/legal/van-ban/{id}` | cây VB public |
| `GET` | `/citizen/legal/khoan/{id}` | Khoản public + nguyên văn |
...
1. Legal ingest/parse/extract + diff  
2. Neo4j + vector Khoản  
3. Admin QA + citation validator  
4. Jobs + RBAC tối thiểu (`admin_*` / `citizen`)  
5. Citizen QA skeleton (cùng engine, filter public)

### Phase B — Admin giám sát

1. Social ingest 1–2 chủ đề  
2. Link 2 tầng + claim labels + alerts  
3. Graph neighborhood API  
4. Review queue  

### Phase C — Citizen hoàn chỉnh + truyền thông

1. ContentBrief + PublishGate  
2. ResponseSuggest từ alert cluster  
3. Citizen News + VB public  
